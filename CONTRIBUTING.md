# Contributing to UX Blocks Extended

> **Read-only mirror.** This repository (`symfinity/ux-blocks-extended`) is published from the Symfinity product monorepo (`packages/ux-blocks-extended/`). **Do not expect pull requests to be merged here.**
...
Open an [issue](https://github.com/symfinity/ux-blocks-extended/issues) on this repository (preferred for discoverability from Packagist and GitHub).
...
Open a pull request on the **product monorepo** (`packages/ux-blocks-extended/`) on a `feature/{spec-id}-{slug}` branch. Do not rely on merging on this mirror.
