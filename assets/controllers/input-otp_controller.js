    ...
    connect() {
        if (!this.hasInputTarget) {
            ...
            for (let i = 0; i < n; i += 1) {
                ...
                input.setAttribute(
                    'data-action',
                    'input->symfony--ux-blocks-extended--input-otp#onInput keydown->symfony--ux-blocks-extended--input-otp#onKeydown paste->symfony--ux-blocks-extended--input-otp#onPaste',
                );
    ...
    _syncHidden() {
        ...
        if (code.length === this.lengthValue) {
            this.element.dispatchEvent(
                new CustomEvent('ux-blocks-extended:input-otp-complete', {
                    bubbles: true,
                    detail: { code },
                }),
            );
