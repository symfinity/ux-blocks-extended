...
export default class extends Controller {
    ...
    connect() {
        if (this.starTargets.length === 0) {
            for (let i = 1; i <= this.maxValue; i += 1) {
                ...
                button.setAttribute('data-symfony--ux-blocks-extended--rating-target', 'star');
                button.setAttribute(
                    'data-action',
                    'click->symfony--ux-blocks-extended--rating#select mouseenter->symfony--ux-blocks-extended--rating#preview mouseleave->symfony--ux-blocks-extended--rating#clearPreview',
                );
    ...
    _emitChange(value) {
        this.element.dispatchEvent(
            new CustomEvent('ux-blocks-extended:rating-change', {
                bubbles: true,
                detail: { value },
            }),
        );
