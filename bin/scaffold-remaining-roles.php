...
function scaffoldMenu(string $role, string $component, string $controllerSlug, array $nested = ['Trigger', 'Content', 'Item']): void
{
    ...
    $kebab = $role;
    $ctrl = 'symfony--ux-blocks-extended--' . $controllerSlug;

    writeFile(
        "{$src}/{$component}.php",
        phpClass($component, $component, "@UxBlocksExtended/components/{$component}.html.twig"),
    );
    writeFile(
...
<div
    data-ui-role="{$kebab}"
    data-ui-fragment="blocks.ext.{$kebab}"
...
TWIG,
    ...
    foreach ($nested as $part) {
        ...
        writeFile(
            "{$src}/{$class}.php",
            phpClass($class, $twigPart, "@UxBlocksExtended/components/{$component}/{$part}.html.twig"),
        );
...
<{$rootTag}
    data-ui-role="{$role}"
    data-ui-fragment="blocks.ext.{$role}"
    data-controller="{$ctrl}"
    {$extraAttrs}
    {{ attributes }}
>
...
$role = 'sidebar';
$component = 'Sidebar';
$ctrl = 'symfony--ux-blocks-extended--sidebar';
writeFile("{$src}/{$component}.php", phpClassWithProps($component, $component, "@UxBlocksExtended/components/{$component}.html.twig", "    public string \$side = 'left';\n"));
writeFile("{$tpl}/{$component}.html.twig", <<<TWIG
<div
    data-ui-role="sidebar"
    data-ui-fragment="blocks.ext.sidebar"
    data-controller="{$ctrl}"
    data-symfony--ux-blocks-extended--sidebar-side-value="{{ side }}"
    {{ attributes }}
>
    {% block content %}{% endblock %}
</div>
...
foreach (['Trigger' => 'button', 'Content' => 'aside', 'Header' => 'div', 'Title' => 'h2'] as $part => $tag) {
    ...
    writeFile("{$src}/{$class}.php", phpClass($class, "{$component}:{$part}", "@UxBlocksExtended/components/{$component}/{$part}.html.twig"));
    $attrs = $part === 'Trigger'
        ? 'type="button" aria-expanded="false" data-symfony--ux-blocks-extended--sidebar-target="trigger"'
        : ($part === 'Content' ? 'data-symfony--ux-blocks-extended--sidebar-target="content"' : '');
...
// Forms
scaffoldSimple('slider', 'Slider', 'slider', 'div', 'data-symfony--ux-blocks-extended--slider-target="track"');
writeFile("{$tpl}/Slider.html.twig", <<<TWIG
<div
    data-ui-role="slider"
    data-ui-fragment="blocks.ext.slider"
    data-controller="symfony--ux-blocks-extended--slider"
    {{ attributes }}
>
    <input type="range" data-symfony--ux-blocks-extended--slider-target="input" min="0" max="100" value="50" />
    {% block content %}{% endblock %}
</div>
...
writeFile("{$src}/Calendar.php", phpClass('Calendar', 'Calendar', '@UxBlocksExtended/components/Calendar.html.twig'));
writeFile("{$tpl}/Calendar.html.twig", <<<TWIG
<div
    data-ui-role="calendar"
    data-ui-fragment="blocks.ext.calendar"
    data-controller="symfony--ux-blocks-extended--calendar"
    {{ attributes }}
>
    <div data-ui-part="grid" data-symfony--ux-blocks-extended--calendar-target="grid"></div>
    {% block content %}{% endblock %}
</div>
...
writeFile("{$src}/InputOtp.php", phpClassWithProps('InputOtp', 'InputOtp', '@UxBlocksExtended/components/InputOtp.html.twig', "    public int \$length = 6;\n"));
writeFile("{$tpl}/InputOtp.html.twig", <<<TWIG
<div
    data-ui-role="input-otp"
    data-ui-fragment="blocks.ext.input-otp"
    data-controller="symfony--ux-blocks-extended--input-otp"
    data-symfony--ux-blocks-extended--input-otp-length-value="{{ length }}"
    {{ attributes }}
>
    {% block content %}{% endblock %}
</div>
    ...
    connect() {
        if (!this.hasInputTarget) {
            ...
            for (let i = 0; i < n; i += 1) {
                ...
                input.maxLength = 1;
                input.setAttribute('data-symfony--ux-blocks-extended--input-otp-target', 'input');
    ...
    connect() {
        if (this.starTargets.length === 0) {
            for (let i = 1; i <= this.maxValue; i += 1) {
                ...
                btn.type = 'button';
                btn.textContent = '★';
                btn.setAttribute('data-symfony--ux-blocks-extended--rating-target', 'star');
...
writeFile("{$src}/{$component}.php", phpClass($component, $component, "@UxBlocksExtended/components/{$component}.html.twig"));
writeFile("{$tpl}/{$component}.html.twig", <<<TWIG
<div
    data-ui-role="carousel-interactive"
    data-ui-fragment="blocks.ext.carousel-interactive"
    data-controller="symfony--ux-blocks-extended--carousel-interactive"
    {{ attributes }}
>
    <div data-ui-role="carousel-interactive-viewport" data-symfony--ux-blocks-extended--carousel-interactive-target="viewport">
        {% block content %}{% endblock %}
    </div>
</div>

TWIG);
writeFile("{$src}/CarouselInteractiveItem.php", phpClass('CarouselInteractiveItem', 'CarouselInteractive:Item', '@UxBlocksExtended/components/CarouselInteractive/Item.html.twig'));
writeFile("{$tpl}/CarouselInteractive/Item.html.twig", '<div data-ui-role="carousel-interactive-item" data-symfony--ux-blocks-extended--carousel-interactive-target="item" {{ attributes }}>{% block content %}{% endblock %}</div>' . "\n");
...
<div
    ...
    data-ui-fragment="blocks.ext.resizable"
    data-controller="symfony--ux-blocks-extended--resizable"
    {{ attributes }}
>
    {% block content %}{% endblock %}
</div>
...
foreach (['Panel', 'Handle'] as $part) {
    ...
    writeFile("{$src}/{$class}.php", phpClass($class, "{$component}:{$part}", "@UxBlocksExtended/components/{$component}/{$part}.html.twig"));
    writeFile("{$tpl}/{$component}/{$part}.html.twig", "<div data-ui-role=\"{$sub}\" data-symfony--ux-blocks-extended--resizable-target=\"" . strtolower($part) . "\" {{ attributes }}>{% block content %}{% endblock %}</div>\n");
}
...
writeFile("{$src}/Toast.php", phpClass('Toast', 'Toast', '@UxBlocksExtended/components/Toast.html.twig'));
writeFile("{$tpl}/Toast.html.twig", <<<TWIG
<div
    data-ui-role="toast"
    data-ui-fragment="blocks.ext.toast"
    data-controller="symfony--ux-blocks-extended--toast"
    aria-live="polite"
    {{ attributes }}
>
    {% block content %}{% endblock %}
...
TWIG);
writeFile("{$src}/ToastItem.php", phpClass('ToastItem', 'Toast:Item', '@UxBlocksExtended/components/Toast/Item.html.twig'));
writeFile("{$tpl}/Toast/Item.html.twig", '<div data-ui-role="toast-item" data-symfony--ux-blocks-extended--toast-target="item" {{ attributes }}>{% block content %}{% endblock %}</div>' . "\n");
...
export default class extends Controller {
    ...
    show(message) {
        ...
        el.setAttribute('data-symfony--ux-blocks-extended--toast-target', 'item');
...
writeFile("{$src}/CommandPalette.php", phpClassWithProps(
    'CommandPalette',
    'CommandPalette',
    '@UxBlocksExtended/components/CommandPalette.html.twig',
    "    public string \$commandsUrl = '';\n    public string \$placeholder = 'Search…';\n",
));
writeFile("{$tpl}/CommandPalette.html.twig", <<<TWIG
<div
    data-ui-role="command-palette"
    data-ui-fragment="blocks.ext.command-palette"
    data-controller="symfony--ux-blocks-extended--command-palette"
    data-symfony--ux-blocks-extended--command-palette-commands-url-value="{{ commandsUrl }}"
    data-symfony--ux-blocks-extended--command-palette-placeholder-value="{{ placeholder }}"
    hidden
    {{ attributes }}
>
    {% block content %}{% endblock %}
...
foreach (['Input' => 'input', 'List' => 'list'] as $part => $partRole) {
    ...
    writeFile("{$src}/{$class}.php", phpClass($class, "CommandPalette:{$part}", "@UxBlocksExtended/components/CommandPalette/{$part}.html.twig"));
    writeFile("{$tpl}/CommandPalette/{$part}.html.twig", "<div data-ui-part=\"{$partRole}\" data-ui-role=\"command-palette-{$partRole}\" data-symfony--ux-blocks-extended--command-palette-target=\"{$partRole}\" {{ attributes }}>{% block content %}{% endblock %}</div>\n");
}
writeFile("{$src}/CommandPaletteItem.php", phpClass('CommandPaletteItem', 'CommandPalette:Item', '@UxBlocksExtended/components/CommandPalette/Item.html.twig'));
writeFile("{$tpl}/CommandPalette/Item.html.twig", '<button type="button" data-ui-role="command-palette-item" data-symfony--ux-blocks-extended--command-palette-target="item" {{ attributes }}>{% block content %}{% endblock %}</button>' . "\n");
...
export default class extends Controller {
    ...
    _render() {
        ...
        this._commands.forEach((cmd) => {
            ...
            btn.setAttribute('data-symfony--ux-blocks-extended--command-palette-target', 'item');
...
<div
    ...
    data-ui-fragment="blocks.ext.stacked-layout-interactive"
    data-controller="symfony--ux-blocks-extended--stacked-layout-interactive"
    {{ attributes }}
>
    <nav data-ui-part="nav" data-symfony--ux-blocks-extended--stacked-layout-interactive-target="nav">
        {% block nav %}{% endblock %}
    </nav>
    <main>
        {% block content %}{% endblock %}
